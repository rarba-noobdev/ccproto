              ...
              <div className="p-6 sm:p-8 lg:p-10">
                ...
                <div className="mt-7 max-w-2xl rounded-full border border-[var(--line)] bg-[var(--surface-2)] p-2 shadow-[inset_0_1px_0_rgba(255,255,255,.8)]">
                  <Link to="/gaming-pcs" className="group flex min-h-12 items-center gap-3 rounded-full bg-white px-4 transition hover:bg-[var(--canvas)]">
                    ...
                    <span className="flex-1 text-left text-sm font-bold text-[var(--ink-muted)]">Search graphics cards, SSDs, monitors…</span>
                ...
                <div className="mt-6 grid gap-2 sm:grid-cols-3">
                  {[
                    ...
                    <div key={title} className="rounded-[20px] border border-[var(--line)] bg-[var(--surface-2)] p-4">
                      ...
                      <div className="text-sm font-bold tracking-[-.02em]">{title}</div>
              ...
              >
                <Link to={to} className="group flex min-h-24 flex-col justify-between rounded-[24px] border border-[var(--line)] bg-[var(--surface-1)] p-4 transition hover:-translate-y-0.5 hover:bg-[var(--surface-hover)]">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-lg font-bold tracking-[-.04em]">{label}</span>
          ...
          <div className="rounded-[30px] border border-[var(--line)] bg-[var(--surface-1)] p-5">
            <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="text-4xl font-bold leading-none tracking-[-.065em]">Popular paths</h2>
              </div>
              <Link to="/gaming-pcs" className="btn-secondary w-fit">Browse all</Link>
            </div>
            <div className="flex flex-wrap gap-2">
              {wanted.map(([label, to]) => (
                <Link key={label} to={to} className="rounded-full border border-[var(--line)] bg-[var(--surface-2)] px-4 py-2 text-sm font-bold transition hover:-translate-y-0.5 hover:bg-[var(--surface-hover)]">
                  {label}
                </Link>
              ))}
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[.8fr_1.2fr]">
            <div className="spotlight spotlight-orange min-h-[300px] p-6 text-white">
              <div className="relative z-10 flex h-full flex-col justify-between">
                <p className="text-xs font-bold uppercase tracking-[.12em] text-white/72">Guided build</p>
                <div>
                  <h2 className="max-w-sm text-5xl font-bold leading-[.9] tracking-[-.07em]">Build without guessing.</h2>
                  <Link to="/build" className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-black">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                <article key={title} className="rounded-[26px] border border-[var(--line)] bg-[var(--surface-1)] p-5">
                  ...
                  <h3 className="text-2xl font-bold tracking-[-.05em]">{title}</h3>
          ...
          {(testimonials.length ? testimonials.slice(0, 3) : [
            ...
            <blockquote key={t.id} className="rounded-[26px] border border-[var(--line)] bg-[var(--surface-1)] p-5">
              <div className="mb-5 flex items-center justify-between">
                <span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--ink)] text-xs font-bold text-[var(--canvas)]">{t.initials}</span>
...
function SectionHeader({ title, to }) {
  return (
    <div className="mb-5 flex items-end justify-between gap-4">
      <h2 className="text-4xl font-bold leading-none tracking-[-.065em]">{title}</h2>
...
function SystemProduct({ pc, index }) {
  ...
  return (
    <Link to="/prebuilt" className="group overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--surface-1)] transition hover:-translate-y-0.5 hover:border-[var(--line-strong)]">
      <div className="relative grid h-40 place-items-center overflow-hidden bg-[#f6f3ed]">
        <div className="absolute inset-x-8 bottom-3 h-8 rounded-full bg-black/10 blur-xl" aria-hidden="true" />
        <span className="absolute left-3 top-3 rounded-full bg-[var(--ink)] px-2.5 py-1 text-[10px] font-bold text-[var(--canvas)]">#{index + 1}</span>
        {image && <img src={image} alt={pc.name} width="320" height="240" className="relative max-h-[124px] w-[76%] max-w-[220px] object-contain drop-shadow-[0_12px_14px_rgba(0,0,0,.15)] transition duration-300 group-hover:scale-[1.025]" loading="lazy" decoding="async" />}
      </div>
      <div className="p-4">
        <h3 className="line-clamp-2 min-h-11 text-[15px] font-bold leading-[21px]">{pc.name}</h3>
        <div className="mt-3 flex items-center gap-1 text-xs font-bold text-[var(--warning)]">
          <Star className="h-3.5 w-3.5 fill-current" /> 4.7/5
        </div>
        <div className="price mt-3 text-2xl font-bold">{formatINR(pc.price)}</div>
  ...
  return (
    <Link to="/gaming-pcs" className="group overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--surface-1)] transition hover:-translate-y-0.5 hover:border-[var(--line-strong)]">
      <div className="relative grid h-44 place-items-center bg-[#f6f3ed]">
        <span className="absolute left-3 top-3 rounded-full bg-[var(--accent-heat)] px-2.5 py-1 text-[10px] font-bold text-white">
      ...
      <div className="p-4">
        <div className="truncate text-[11px] font-bold uppercase tracking-[.1em] text-[var(--ink-muted)]">{part.brand || part.category}</div>
        <h3 className="mt-1 line-clamp-2 min-h-10 text-sm font-bold leading-5">{part.name}</h3>
        <div className="price mt-3 text-xl font-bold">{formatINR(part.price)}</div>
        {save > 0 && <div className="mt-1 text-xs font-bold text-[var(--success)]">Save {formatINR(save)}</div>}
      </div>
    </Link>
  )
}
